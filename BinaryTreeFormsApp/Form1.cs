    ...
    public partial class Form1 : Form
    {
        ...
        private void buttonGenerateRandom_Click(object sender, EventArgs e)
        {
            if (int.TryParse(textBoxInput.Text, out int count))
            {
                ...
                AdjustPictureBoxSize();
                pictureBox.Invalidate();
                CenterTreeInPanel();  // Центрируем дерево в панели
        ...
        private void AdjustPictureBoxSize()
        {
            if (tree != null && tree.Root != null)
            {
                // Рассчитываем максимальную глубину и ширину дерева
                int treeDepth = tree.GetMaxDepth();  // Метод для получения максимальной глубины дерева
                int treeWidth = (int)Math.Pow(2, treeDepth) * 100;  // Примерный расчёт ширины, основанный на количестве узлов на максимальном уровне

                // Устанавливаем новые размеры PictureBox
                pictureBox.Width = Math.Max(treeWidth, panel.Width);  // Ширина должна быть либо рассчитанной, либо как минимум шириной панели
                pictureBox.Height = treeDepth * 80 + 100;  // Высота увеличивается в зависимости от глубины дерева

                // Обновляем минимальные размеры для прокрутки панели
